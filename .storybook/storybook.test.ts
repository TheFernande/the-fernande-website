...
describe(options.suite, () => {
  getAllStoryFiles().forEach(({ storyFile, filePath }) => {
    ...
    // This is while we find the right way to get the mocks working with the stories in a Jest context.
    if (filePath.includes("src/components/layout/FcbCarousel/FcbCarousel.stories.tsx")) {
      console.warn(`Skipping story: layout/FcbCarousel`);
      return;
    }
